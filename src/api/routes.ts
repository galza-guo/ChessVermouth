...
export function registerRoutes(app: express.Express, deps: RoutesDeps) {
...
function formatAnalysisResponse(result: { bestmove: string; lines: { multipv: number; depth?: number; score?: Score; pv: string[] }[] }) {
  return {
...
function classify(before?: Score, after?: Score): 'blunder' | 'mistake' | 'inaccuracy' | 'good' {
  ...
  if (beforeScore === null || afterScore === null) return 'good';
  const drop = beforeScore - afterScore;
  if (drop >= 150) return 'blunder';
  if (drop >= 80) return 'mistake';
  if (drop >= 30) return 'inaccuracy';
  return 'good';
}

function isMateDrop(before: Score, after: Score): boolean {
  if (before.type === 'mate' && before.value > 0) {
    if (after.type !== 'mate') return true;
