...
export function parseInfoLine(line: string): InfoLine | null {
  ...
  for (let i = 1; i < tokens.length; i++) {
    ...
    switch (token) {
      case 'depth': {
        const next = tokens[++i];
        if (next) info.depth = Number.parseInt(next, 10);
        break;
      }
      case 'seldepth': {
        const next = tokens[++i];
        if (next) info.seldepth = Number.parseInt(next, 10);
        break;
      }
      case 'multipv': {
        const next = tokens[++i];
        if (next) info.multipv = Number.parseInt(next, 10);
        break;
      }
      case 'score': {
        const type = tokens[++i];
      ...
      case 'nps': {
        const next = tokens[++i];
        if (next) info.nps = Number.parseInt(next, 10);
        break;
      }
      case 'nodes': {
        const next = tokens[++i];
...
export function scoreToNumber(score: Score | undefined): number | null {
...
export function invertScore(score: Score | undefined): Score | undefined {
...
export function cloneScore(score: Score | undefined): Score | undefined {
  if (!score) return undefined;
